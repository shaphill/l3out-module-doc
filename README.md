...
## Guided Example
### Configuration
![Topology](https://i.imgur.com/PUYRtEO.png)
...
```
#### Prerequisites
In order to create this L3Out, you must have the appropriate policy objects in place. In this document we will not cover the creation of Tenant, VRF, BD, Application Profiles, or Access Policies (L3 Domain etc).  For resources on how to configure these objects, view documentation [here](https://www.cisco.com/c/en/us/td/docs/dcn/aci/apic/6x/l3-configuration/cisco-apic-layer-3-networking-configuration-guide-60x.html). The full Terraform configuration for this demo, including variable files and Tenant configuration is available [here](https://github.com/shaphill/l3out-module-demo).
...
```
For the OSPF configuration, we create the [ospf](#input\_ospf) block and pass in our parameters---in our case we are just using regular area 0.
```
  ospf  =  {
    area_id    =  "0"
    area_type  =  "regular"
...
```
We will use the [vPCs](#input\_vpcs) variable block to define all  interface and node profiles, including SVI configuration. This configuration doesn't use an OSPF Interface Profile, so we leave the `ospf_interface_profile` blank here. If you want to use an OSPF interface policy, you must create the [aci_ospf_interface_policy](https://registry.terraform.io/providers/CiscoDevNet/aci/latest/docs/resources/ospf_interface_policy) resource outside of the L3Out module and add it in the `ospf_interface_profile` block as `ospf_interface_policy = aci_ospf_interface_policy.{name}.id`
```
  vpcs  =  [
    {
      ospf_interface_profile  = {
        //add interface profile here
      }
      pod_id  =  1
      nodes  = [
        {
...
```
Lastly, we specify the L3 EPG that will be receiving traffic from our host EPG. We also define our external subnets and a consumed contract to permit traffic. 
```
  external_epgs  =  [
    {
      name = "l3-ospf-epg"
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTIwMDI3MTc4LDU1ODc5MjAxNywtNjgzOD
QxODQ2LDUwODcyNjU5Miw4NDk4MDE3ODIsLTEyMjIyMzkxMjEs
LTUzNjEyNzc1LC0xNDIwNTkwNTc0LC00MTY3MDkyMjQsODA2OD
QxOTc4LDE3NDg4NjkxOTIsLTIxNDA0MjY1NjMsODA3Njc1Njks
MTY4ODQ5ODgyNiwtMTY0NTE5MTE3NywxMTY1NjExMzA4LDEyNj
k5OTkzMzIsLTUzOTYyNjE5MywxODAzNTc4NjUsLTE2NjkyMTU2
MDhdfQ==
-->